...
function rawId(r: Rec) { return text(r,["id","bookId","book_id","dramaId","drama_id","seriesId","series_id","videoId","video_id","contentId","content_id","slug","code","key"]); }
...
function cover(r: Rec, depth=0): string { if(depth>4)return""; const v=text(r,["cover","coverUrl","cover_url","poster","posterUrl","poster_url","image","imageUrl","image_url","thumbnail","thumb"]); if(/^https?:\/\//i.test(v))return v; for(const x of Object.values(r)){if(rec(x)){const c=cover(x,depth+1);if(c)return c}} return ""; }
function records(v: unknown, depth=0): Rec[] { if(depth>7)return[]; if(Array.isArray(v)){const direct=v.filter(rec).filter(x=>rawId(x)&&title(x)); if(direct.length)return direct; return v.flatMap(x=>records(x,depth+1));} if(rec(v)){for(const k of["data","list","items","results","books","dramas","series","videos","content"]){if(k in v){const f=records(v[k],depth+1);if(f.length)return f}} return Object.values(v).flatMap(x=>records(x,depth+1));} return[]; }
function episodeRecords(v: unknown, depth=0): Rec[] { if(depth>7)return[]; if(Array.isArray(v)){const rs=v.filter(rec); const eps=rs.filter(r=>numberValue(r,["episode","episodeNo","episode_no","ep","chapter","chapterNo","chapter_no","index"])||text(r,["episodeId","episode_id","chapterId","chapter_id","id"])); if(eps.length)return eps; return v.flatMap(x=>episodeRecords(x,depth+1));} if(rec(v)){for(const k of["episodes","chapters","episodeList","episode_list","chapterList","chapter_list","list","items","data"]){if(k in v){const f=episodeRecords(v[k],depth+1);if(f.length)return f}} return Object.values(v).flatMap(x=>episodeRecords(x,depth+1));} return[]; }
function stream(v: unknown, depth=0): string { if(depth>8)return""; if(typeof v==="string"&&/^https?:\/\//i.test(v)&&/(\.m3u8|\.mp4|stream|video)/i.test(v))return v; if(Array.isArray(v)){for(const x of v){const s=stream(x,depth+1);if(s)return s}} else if(rec(v)){for(const k of["hls_url","hlsUrl","m3u8","m3u8_url","videoUrl","video_url","streamUrl","stream_url","url"]){const s=stream(v[k],depth+1);if(s)return s}for(const x of Object.values(v)){const s=stream(x,depth+1);if(s)return s}} return""; }
...
function query(path:string,id:string,episode?:Episode){const qs=new URLSearchParams(); qs.set("id",id); qs.set("bookId",id); qs.set("dramaId",id); qs.set("seriesId",id); if(episode){qs.set("episode",String(episode.number));qs.set("ep",String(episode.number));qs.set("episodeId",episode.sourceId||String(episode.number));qs.set("chapterId",episode.sourceId||String(episode.number));} return `${path}?${qs}`;}
function normalizeEpisode(r:Rec,i:number):Episode{const n=numberValue(r,["episode","episodeNo","episode_no","ep","chapter","chapterNo","chapter_no","index"])||i+1;const sid=text(r,["episodeId","episode_id","chapterId","chapter_id","id"])||String(n);return{id:sid,sourceId:sid,number:n,title:text(r,["title","name","episodeTitle","episode_title","chapterTitle","chapter_title"])||`Episode ${n}`,streamUrl:stream(r)||undefined};}
function normalizeDrama(r:Rec,slug:string,name:string,episodes:Episode[]=[]):Drama|null{const id=rawId(r),t=title(r);if(!id||!t)return null;const c=cover(r)||"https://placehold.co/720x1280/111111/eeeeee?text=DRACIN";return{id:`sansekai:${slug}--${id}`,source:"captain",provider:slug,providerName:`${name} · Sansekai`,sourceId:id,title:t,synopsis:text(r,["synopsis","description","desc","intro","summary"])||"Sinopsis belum tersedia.",genre:text(r,["genre","category","tags"])||"Drama",cover:c,backdrop:c,episodes};}
