...
function findDramaObjects(value: unknown, depth = 0): JsonRecord[] {
  ...
  if (isRecord(value)) {
    for (const key of ["data", "list", "items", "dramas", "books", "results", "recommend", "recommendations", "records", "content", "rows"]) {
      if (key in value) {
...
function episodeArray(value: unknown, depth = 0): JsonRecord[] {
  ...
  if (Array.isArray(value)) {
    ...
    const episodeLike = records.filter((item) => Boolean(numberValue(item, ["episode", "episode_no", "episodeNo", "ep", "ep_num", "epNum", "chapter_num", "chapterNum", "index", "sort", "episode_index", "episodeIndex"]) || text(item, ["episode_id", "episodeId", "chapter_id", "chapterId", "video_id", "videoId"])));
    if (episodeLike.length) return episodeLike;
    return value.flatMap((item) => episodeArray(item, depth + 1));
  }
  if (isRecord(value)) {
    for (const key of ["episodes", "episode_list", "episodeList", "chapters", "chapter_list", "chapterList", "video_list", "videoList", "list", "items", "data", "content"]) {
      if (key in value) {
...
function normalizeEpisode(item: JsonRecord, index: number): Episode {
  const number = numberValue(item, ["episode", "episode_no", "episodeNo", "ep", "ep_num", "epNum", "chapter_num", "chapterNum", "index", "sort", "episode_index", "episodeIndex"]) || index + 1;
...
export async function getDrama(id: string): Promise<Drama | null> {
  ...
  try {
    ...
    if (episodes.length === 0) {
      const total = numberValue(record, ["episode_count", "episodeCount", "total_episode", "totalEpisode", "total_episodes", "totalEpisodes", "chapter_count", "chapterCount"]);
      if (total > 0 && total <= 500) {
        episodes = Array.from({ length: total }, (_, i) => ({ id: `ep-${i + 1}`, sourceId: String(i + 1), number: i + 1, title: `Episode ${i + 1}` }));
      }
    }
...
export function extractStreamUrl(value: unknown, depth = 0): string {
  ...
  if (isRecord(value)) {
    for (const key of ["m3u8", "m3u8_url", "m3u8Url", "video_url", "videoUrl", "play_url", "playUrl", "stream_url", "streamUrl", "url", "src", "playAddress", "play_address"]) {
...
export async function getEpisodeStream(drama: Drama, episodeNumber: number): Promise<string | null> {
  ...
  try {
    const payload = await captainJson(`/freereels/api/v1/dramas/${encodeURIComponent(drama.sourceId)}/play/${episodeNumber}?lang=${LANGUAGE}`);
